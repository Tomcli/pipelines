...
func (p *WorkflowFormatter) Format(workflow *v1alpha1.PipelineRun) error {
...
func getWorkflowName(workflow *v1alpha1.PipelineRun) string {
...
func (p *WorkflowFormatter) formatWorkflowParameters(workflow *v1alpha1.PipelineRun) error {
	if workflow.Spec.Params == nil {
		return nil
	}

	newParams := make([]v1alpha1.Param, 0)
...
func (p *WorkflowFormatter) formatParameter(param v1alpha1.Param) (*v1alpha1.Param, error) {
	formatted, err := p.formatString(param.Value.StringVal)
	if err != nil {
		return nil, err
	}

	tektonFormatted := v1alpha1.ArrayOrString{
		Type:      "string",
		StringVal: formatted,
	}
